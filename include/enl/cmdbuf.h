...
*/
template<class T, unsigned nSize, bool bClearOnEndRead = false, bool 
bSortBeforeCopy = false>
...
public:
	...
	void BeginWrite()
	{
		...
		//std::cerr << "CMDBUF-W: Back buffer locked" << 
std::endl;
	...
	void Write(T& cmd)
	{
		...
		{
			std::cerr << "CMDBUF-W: Back buffer is full!!!" 
<< std::endl;
			return;
		}
		memcpy(m_pBackBuffer + m_nBackBufferCount, &cmd, 
sizeof(T));

		unsigned char* pCmd = (unsigned char*)&cmd;
		for (int i = 0; i < sizeof(T); i++)
		{
			m_nChecksum = (m_nChecksum >> 1) + ((m_nChecksum 
& 1) << 15);
			m_nChecksum += pCmd[i];
			m_nChecksum &= 0xFFFF;
		}
	...
	void EndWrite()
	{
		...
		//std::cerr << "CMDBUF-W: Back buffer unlocked" << 
std::endl;
		Copy();
	}
	...
	bool BeginRead(T** pCmdBuf, size_t* nCount)
	{
		...
		if (!bLocked)
			return false;
		//std::cerr << "CMDBUF-R: Front buffer locked" << 
std::endl;
	...
	void EndRead()
	{
		...
		//std::cerr << "CMDBUF-R: Front buffer unlocked" << 
std::endl;
	}
...
private:
	void Sort()
	{
		...
		{
			size_t j = i;
			while (j > 0 && _BBSORTKEY(j - 1) > 
_BBSORTKEY(j))
			{
				T t;
				memcpy(&t, m_pBackBuffer + j, 
sizeof(T));
				memcpy(m_pBackBuffer + j, m_pBackBuffer 
- 1, sizeof(T));
				memcpy(m_pBackBuffer + j, m_pBackBuffer 
+ (j - 1), sizeof(T));
				memcpy(m_pBackBuffer + (j-1), &t, 
sizeof(T));
				j--;
			}
			i++;
	...
	{
		m_BackBufferLock.lock();
		//std::cerr << "CMDBUF-C: Back buffer locked" << 
std::endl;
		m_FrontBufferLock.lock();
		//std::cerr << "CMDBUF-C: Front buffer locked" << 
std::endl;
		if (m_bForceCopy || (m_nChecksum != m_nLastChecksum))
		{
			...
			memcpy(m_pFrontBuffer, m_pBackBuffer, 
m_nBackBufferCount * sizeof(T));
			m_nFrontBufferCount = m_nBackBufferCount;
			m_bForceCopy = false;
		}
		else
		{
			//std::cerr << "CMDBUF-C: No copy needed" << 
std::endl;
		}
		m_nFrontBufferCount = m_nBackBufferCount;
		m_nLastChecksum = m_nChecksum;
		m_FrontBufferLock.unlock();
		//std::cerr << "CMDBUF-C: Front buffer unlocked" << 
std::endl;
		m_BackBufferLock.unlock();
		//std::cerr << "CMDBUF-C: Back buffer unlocked" << 
std::endl;
	}
...
private:
	...
	std::mutex m_BackBufferLock; // prevents copying to or from the 
back buffer; locked by BeginWrite or BeginCopy; opened by EndWrite or 
EndCopy
	std::mutex m_FrontBufferLock; // prevents copying to or from the 
front buffer; locked by BeginRead or BeginCopy; opened by EndRead or 
EndCopy
